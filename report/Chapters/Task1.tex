...
and has a known optimal tour length of 21{,}282. The dimension falls within
the 70--150 range required by the project brief.

kroA100 is one of the most cited TSPLIB instances in the GA-for-TSP
literature, so its difficulty level and optimum are well documented. At
...
instance is small enough to support a 54-configuration parameter sweep with
30 runs each within a practical compute budget. Having a known optimum also
...
The search space is discrete and finite. Once we quotient out rotational and
reflective symmetries, its cardinality is $n!/(2n)$, which for $n=100$ is
approximately $4.7 \times 10^{155}$. This factorial growth rules out exact
search and motivates a population-based metaheuristic such as a Genetic
Algorithm~\cite{holland1975adaptation,goldberg1989genetic}. The fitness
landscape is non-convex and multimodal --- many tours share similar lengths
but differ structurally in the edges they use --- which is where recombination
of partial tours can outperform single-point local search. The permutation
constraint also dictates the operator design. Classical string-style crossover
and bit-flip mutation do not preserve the permutation property, so we must
either use feasibility-preserving operators or pair an unconstrained operator
with an explicit repair step. We take the latter route so that repair can be
studied as an explicit experimental factor, following the constraint-handling
taxonomy in Larra{\~n}aga et al.~\cite{larranaga1999genetic}.

\section{Experimental-Setup Justification (Task 1.3)}
\label{T1:Setup}

We fix the experimental protocol before collecting any results so that every
configuration is evaluated on identical terms.

Each configuration is run for \textbf{30 independent random seeds}. Thirty
seeds gives a reasonable sample for reporting means and standard deviations
while keeping the full sweep tractable.
...
300 cover the long tail where late mutations or rare crossover events still
trim a few edges. A fixed generation budget, rather than an adaptive stopping
...
al.~\cite{larranaga1999genetic}, letting the sweep separate exploration
pressure from disruption. We include selection method at two levels because
tournament and roulette selection apply qualitatively different pressure
profiles, and the interaction between selection pressure and mutation rate is
one of the questions \cref{Task4} addresses.

Combining the sweep with 30 seeds gives $54 \times 30 = 1{,}620$ total runs,
which is enough to support non-parametric tests and a per-factor sensitivity
analysis.

A second group of parameters is held fixed across the sweep because we want
to isolate the four factors above rather than explore the full operator space.
...
enabled, and mutation uses the swap operator. Choosing naive single-point
crossover with repair is itself an experimental decision: it guarantees that
every offspring needs repair, which is the configuration \cref{Task2} and
