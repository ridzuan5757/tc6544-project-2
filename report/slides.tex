...
PMX copies a segment from Parent A, then uses a mapping to place the
remaining values from Parent B without creating duplicates.
...
every offspring. This way, repair has to do real work on every generation.
...
shuffle missing cities, place at duplicate positions. Does not favour
any particular city arrangement, so population diversity is preserved.
...
\smallskip
Repair and PMX are almost the same; penalty is ${\sim}16\%$ worse.
\end{frame}

\begin{frame}{Takeaways}
\begin{enumerate}
    \item Naive crossover + repair performs almost as well as PMX on
          kroA100 (only ${\sim}2\%$ difference)
    \item Random insertion keeps more diversity than deterministic
          repair strategies
    \item Penalty function is ${\sim}16\%$ worse than both
          feasibility-based approaches
    \item Fixing or preventing infeasibility works much better than
          just penalising it
\end{enumerate}
